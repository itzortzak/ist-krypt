...
function loadStats(){
  try{
    const stats = JSON.parse(localStorage.getItem("ik_stats")||'{"solved":0,"correct":0,"wrong":0,"reveals":0,"chapterSolved":{}}');
    if(!stats.chapterSolved) stats.chapterSolved = {};
    return stats;
  }catch(e){
    return {solved:0,correct:0,wrong:0,reveals:0,chapterSolved:{}};
  }
}
...
function renderPuzzle(){
  ...
  const ci = document.getElementById("chapterIcon"); if(ci) ci.textContent = chapterIcon(currentPuzzle.chapter);
  ...
  const qp = document.getElementById("quotePanel"); if(qp){qp.style.display="none"; qp.innerHTML="";}
  const ep = document.getElementById("examPanel"); if(ep){ep.style.display="none"; ep.innerHTML="";}
}
function pickPuzzle(){
...
function checkAnswer(){
  ...
  if(user && user === correct){
    ...
    if(qp){
      ...
      qp.className = "feedback study";
      qp.innerHTML = `<strong>Σύντομη διατύπωση από τη σχολική ύλη:</strong><br>${currentPuzzle.quote || "—"}<br><br><span class="small">${currentPuzzle.source || ""}</span>`;
    }
    const ep = document.getElementById("examPanel");
    if(ep){
      ep.style.display = "block";
      ep.className = "feedback exam";
      ep.innerHTML = `<strong>Σύνδεση με τύπο Πανελλαδικών:</strong> ${currentPuzzle.examType || "Σύντομη απάντηση"}<br><br><strong>Ερώτηση:</strong> ${currentPuzzle.examQuestion || ""}`;
    }
  } else {
...
async function init(){
  try{
    let res = await fetch("./data/puzzles.json", {cache:"no-store"});
    if(!res.ok) res = await fetch("data/puzzles.json", {cache:"no-store"});
    if(!res.ok) throw new Error("Δεν βρέθηκε το αρχείο data/puzzles.json");
    puzzles = await res.json();
    if(!Array.isArray(puzzles) || !puzzles.length) throw new Error("Το puzzles.json είναι κενό ή μη έγκυρο.");
  }catch(err){
    const feedback = document.getElementById("feedback");
    if(feedback){
      feedback.innerHTML = `<div class="feedback err"><strong>Σφάλμα φόρτωσης.</strong> ${err.message}<br><br>Έλεγξε ότι υπάρχει ο φάκελος <code>data</code> και μέσα του το αρχείο <code>puzzles.json</code>.</div>`;
    }
    return;
  }
  populateFilters();
