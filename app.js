...
function auditStatusCounts(){
  const reviewed = puzzles.filter(p=>p.reviewed).length;
  const pending = puzzles.filter(p=>!p.reviewed).length;
...
function exportCurrentDataset(){
  const blob = new Blob([JSON.stringify(puzzles, null, 2)], {type:"application/json;charset=utf-8"});
  const a = document.createElement("a");
  a.href = URL.createObjectURL(blob);
  a.download = "puzzles-audit-export.json";
  a.click();
...
async function init(){
  ...
  document.getElementById("exportBtn")?.addEventListener("click", exportCurrentDataset);
